...
fio \
    ...
    --rw=write \
    --group_reporting=1 \
    --output=$OUT_DIR/$(basename $TEST_DIR)_write_throughput.json \
    --output-format=json
# Clean up
...
fio \
    ...
    --rw=randwrite \
    --group_reporting=1 \
    --output=$OUT_DIR/$(basename $TEST_DIR)_write_iops.json \
    --output-format=json
# Clean up
rm -f $TEST_DIR/write* $TEST_DIR/read*
...
fio \
    ...
    --rw=read \
    --group_reporting=1 \
    --output=$OUT_DIR/$(basename $TEST_DIR)_read_throughput.json \
    --output-format=json
# Clean up
rm -f $TEST_DIR/write* $TEST_DIR/read*
...
fio \
    ...
    --rw=randread \
    --group_reporting=1 \
    --output=$OUT_DIR/$(basename $TEST_DIR)_read_iops.json \
    --output-format=json
